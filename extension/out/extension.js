...
async function checkBackendConnection() {
    try {
        await axios_1.default.get(getBackendUrl());
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('devsentinel.chatView', sidebarProvider));
    // 5. Initial Check
    ...
    context.subscriptions.push(vscode.languages.registerHoverProvider('*', new DevSentinelHoverProvider()));
    // ─── Command: Review File ───
    context.subscriptions.push(vscode.commands.registerCommand('devsentinel.review', async () => {
        ...
        try {
            const config = vscode.workspace.getConfiguration('devsentinel');
            const timeout = config.get('requestTimeout') || 90000;
            const response = await axios_1.default.post(`${getBackendUrl()}/review`, {
            ...
            for (const review of reviews) {
                ...
                const diagnostic = new vscode.Diagnostic(range, `DevSentinel: ${review.suggestion}`, mapSeverity(review.severity || 'warning'));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('devsentinel.roast', async () => {
        ...
        try {
            const config = vscode.workspace.getConfiguration('devsentinel');
            const timeout = config.get('requestTimeout') || 60000;
            const response = await axios_1.default.post(`${getBackendUrl()}/roast`, {
                full_file_content: document.getText(),
                file_path: fileName
            }, { timeout: timeout });
            const roast = response.data.roast;
            sidebarProvider.postMessage({ type: 'roast', file: fileName, roast });
            const panel = vscode.window.createWebviewPanel('devSentinelRoast', '🔥 ROASTED 🔥', vscode.ViewColumn.Beside, {});
    ...
    context.subscriptions.push(vscode.commands.registerCommand('devsentinel.securityScan', async () => {
        ...
        try {
            const config = vscode.workspace.getConfiguration('devsentinel');
            const timeout = (config.get('requestTimeout') || 90000) * 1.5; // Security scan needs more time
            const response = await axios_1.default.post(`${getBackendUrl()}/security-scan`, {
                full_file_content: document.getText(),
                file_path: fileName
            }, { timeout: timeout });
            const reviews = response.data.comments;
            ...
            const newDiags = reviews.map((review) => {
                const lineIndex = review.line_number - 1;
                const range = new vscode.Range(lineIndex, 0, lineIndex, Number.MAX_VALUE);
                const diagnostic = new vscode.Diagnostic(range, `[SECURITY] DevSentinel: ${review.suggestion}`, mapSeverity(review.severity || 'error'));
                diagnostic.source = "DevSentinel";
                diagnostic.code = review.fixed_code;
                return diagnostic;
            });
    ...
    context.subscriptions.push(vscode.commands.registerCommand('devsentinel.complexity', async () => {
        ...
        try {
            const config = vscode.workspace.getConfiguration('devsentinel');
            const timeout = config.get('requestTimeout') || 60000;
            const response = await axios_1.default.post(`${getBackendUrl()}/complexity`, {
                full_file_content: document.getText(),
                file_path: fileName
            }, { timeout: timeout });
            ...
            const complexFunctions = functions.filter((f) => f.is_complex);
            const newDiags = complexFunctions.map((f) => {
                const lineIndex = f.line_number - 1;
                const range = new vscode.Range(lineIndex, 0, lineIndex, Number.MAX_VALUE);
                const diagnostic = new vscode.Diagnostic(range, `[COMPLEXITY] DevSentinel: Function '${f.name}' has cyclomatic complexity of ${f.complexity} (threshold: ${threshold})`, vscode.DiagnosticSeverity.Warning);
                diagnostic.source = "DevSentinel";
                return diagnostic;
            });
            diagnosticCollection.set(document.uri, [...kept, ...newDiags]);
...
// ═══════════════════════════════════════════════════
class DevSentinelSidebarProvider {
...
// Hover Provider
// ═══════════════════════════════════════════════════
class DevSentinelHoverProvider {
    provideHover(document, position, token) {
        const diagnostics = diagnosticCollection.get(document.uri);
        if (!diagnostics || diagnostics.length === 0) {
            return null;
        }
        const lineDiags = diagnostics.filter(d => d.source === 'DevSentinel' && d.range.start.line === position.line);
